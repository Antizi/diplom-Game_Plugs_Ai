...
func _on_save_ml_profile_pressed():
	...
	if str(analytics.config.get("cloud_url", "")).strip_edges().is_empty():
		print("📊 ML профиль сохранён локально (cloud_url не задан — синхронизация с сервером пропущена)")
		return
	if analytics.has_method("sync_game_profile"):
		analytics.sync_game_profile()
	else:
		print("ML профиль сохранён локально")
...
func _on_profile_sync_completed(success: bool, detail: String) -> void:
	if success:
		print("✅ Профиль на сервере обновлён")
	else:
		push_warning("Синхронизация профиля с сервером: " + detail)
